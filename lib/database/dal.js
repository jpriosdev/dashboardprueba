...
async function getFullQAData() {
  ...
  if (!data) {
    ...
    return { summary: {}, bugsByPriority: {}, bugsByModule: [], bugsByCategory: [], developerData: [], sprintData: [], metadata: null, bugs: [] };
  }
...
async function getDataSourceInfo() {
  // Return metadata from JSON if present, otherwise file stats
  try {
    const data = await readJson();
    ...
    if (fsSync.existsSync(DATA_JSON)) {
      ...
      return {
        ...
        totalBugsLoaded: (data && data.summary && (data.summary.totalBugs || data.summary.total_bugs)) || 0,
...
async function getStatistics() {
  ...
  // provide both snake_case and camelCase to be tolerant
  return {
    total_bugs: summary.totalBugs || summary.total_bugs || 0,
    totalBugs: summary.totalBugs || summary.total_bugs || 0,
    total_sprints: (data && data.sprintData && data.sprintData.length) || summary.totalSprints || 0,
    totalSprints: (data && data.sprintData && data.sprintData.length) || summary.totalSprints || 0,
    critical_bugs: summary.criticalBugs || summary.critical_bugs || 0,
    criticalBugs: summary.criticalBugs || summary.critical_bugs || 0,
    pending_bugs: summary.pendingBugs || summary.pending_bugs || 0,
...
async function getBugsBySprint() {
  ...
  return sprints.map((s, idx) => ({
    ...
    sprint_num: Number(s.sprint_num || s.sprint_num === 0 ? s.sprint_num : (s.sprint_num || (s.sprint_num === 0 ? 0 : (s.sprintNumber || idx + 1)))) || (idx + 1),
...
// Lightweight compatibility helpers for runQuery/runScalar
async function runQuery(sql, params = []) {
  // Not supporting SQL on JSON backend; return empty array
  return [];
}

async function runScalar(sql, params = []) {
...
// Minimal implementations for metadata helpers
async function recordDataSourceMetadata() {
  return { success: true };
}
...
function runQuery(sql, params = []) {
  return (async () => {
    await ensureNormalizedView();
    // Use normalized view where possible to cope with DBs that have different column names
    const finalSql = sql.replace(/\bbugs_detail\b/gi, 'bugs_detail_norm');
    return new Promise((resolve, reject) => {
      getDatabase().all(finalSql, params, (err, rows) => {
        if (err) {
          const msg = (err && err.message) ? err.message.toLowerCase() : '';
          if (msg.includes('no such table') || msg.includes('no such column') || msg.includes('no such view') ) {
            console.warn('⚠️ Consulta ignorada por tabla/vista inexistente:', err.message);
            return resolve([]);
          }
          return reject(err);
        }
...
function runScalar(sql, params = []) {
  return (async () => {
    ...
    const finalSql = sql.replace(/\bbugs_detail\b/gi, 'bugs_detail_norm');
    return new Promise((resolve, reject) => {
      getDatabase().get(finalSql, params, (err, row) => {
        if (err) {
          const msg = (err && err.message) ? err.message.toLowerCase() : '';
          if (msg.includes('no such table') || msg.includes('no such column') || msg.includes('no such view') ) {
            console.warn('⚠️ Consulta escalar ignorada por tabla/vista inexistente:', err.message);
            return resolve(null);
          }
          return reject(err);
        }
...
// Create a normalized view `bugs_detail_norm` mapping expected column names
let normalizedViewCreated = false;
function ensureNormalizedView() {
  return new Promise((resolve, reject) => {
    if (normalizedViewCreated) return resolve();
    const db = getDatabase();
    db.all("PRAGMA table_info('bugs_detail')", [], (err, rows) => {
      if (err) return resolve(); // if table doesn't exist, ignore
      const cols = (rows || []).map(r => r.name.toLowerCase());
      const pick = (candidates) => {
        for (const c of candidates) if (cols.includes(c.toLowerCase())) return c;
        return null;
      };
      const mapping = {
        tipo_incidencia: pick(['tipo_incidencia', 'tipo_de_incidencia']),
        clave_incidencia: pick(['clave_incidencia', 'clave_de_incidencia']),
        id_incidencia: pick(['id_incidencia', 'id_de_la_incidencia']),
        resumen: pick(['resumen']),
        prioridad: pick(['prioridad']),
        estado: pick(['estado']),
        sprint: pick(['sprint', 'sprint_de_ejecuci_n', 'sprint_ltima_regresi_n']),
        asignado_a: pick(['asignado_a', 'desarrollador']),
        modulo: pick(['modulo']),
        categoria: pick(['categoria'])
      };
      const selectParts = Object.entries(mapping).map(([alias, col]) => {
        if (col) return `${col} AS ${alias}`;
        return `NULL AS ${alias}`;
      });
      const viewSql = `CREATE VIEW IF NOT EXISTS bugs_detail_norm AS SELECT ${selectParts.join(', ')} FROM bugs_detail`;
      db.run(viewSql, (err2) => {
        // ignore errors creating view; mark created to avoid retry storm
        normalizedViewCreated = true;
        resolve();
      });
    });
  });
