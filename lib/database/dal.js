  ...
  try {
    ...
    try {
      ...
      if (process.env.DEBUG_QA === 'true') {
        try {
          console.log('DEBUG_QA: DAL.readJson read', DATA_JSON, 'size', Buffer.byteLength(txt, 'utf8'));
          console.log('DEBUG_QA: DAL.preview:', txt.slice(0, 800));
          console.log('DEBUG_QA: DAL.parsed.sprintData length', Array.isArray(parsed.sprintData) ? parsed.sprintData.length : 'no-array');
        } catch (e) {}
      }
      return parsed;
    } catch (e) {
        ...
        if (files.length) {
          try {
            ...
            try {
              ...
              // attach a small sample of sprint names or first 2 sprintData entries for easier debug
              try {
                const sprints = parsed2.sprintData || (parsed2.metadata && parsed2.metadata.sprints) || [];
                parsed2._fallbackSample = Array.isArray(sprints) ? sprints.slice(0, 2) : [];
              } catch (exSample) {
                parsed2._fallbackSample = [];
              }
              console.log('ℹ️  DAL fallback: using', files[0].path, 'sample:', parsed2._fallbackSample);
              return parsed2;
            } catch (e2) {
              return {};
            }
