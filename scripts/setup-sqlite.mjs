...
// Si se pasa --generate: crear la BD a partir de data/MockDataV0.csv
if (argv.includes('--generate') || argv.includes('-g')) {
  ...
  // CSV parse básico que soporta comillas
  ...
  (async () => {
    try {
      ...
      // Vistas básicas que el DAL espera
...
console.log('- Ejecuta: node scripts/setup-sqlite.mjs --generate   (crea la DB desde data/MockDataV0.csv)');
